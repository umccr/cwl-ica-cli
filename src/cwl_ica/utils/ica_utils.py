...
# Libica imports
from libica.openapi.libconsole import (
    Configuration as LibConsoleConfiguration,
    ApiClient as LibConsoleApiClient,
    ProjectsApi,
    TokensApi
)
...
from .globals import SCOPES_BY_ROLE, ICA_BASE_URL_ENV_VAR, BASE_URL_NETLOC_REGEX, EXPIRY_DAYS_WARNING_TRIGGER
from .subprocess_handler import run_subprocess_proc
from .conda import get_conda_tokens_dir
from .errors import (
    BaseUrlEnvNotFoundError, InvalidBaseUrlError,
...
def get_projects_list_with_token(base_url, personal_access_token):
    ...
    configuration = LibConsoleConfiguration(
        host=base_url,
        api_key={
    ...
    with LibConsoleApiClient(configuration) as api_client:
...
def create_token_from_api_key_with_role(base_url, api_key, project_id, role):
    ...
    # Set the configuration
    configuration = LibConsoleConfiguration(host=base_url,
                                                            api_key={
                                                                "Authorization": api_key
                                                            })
