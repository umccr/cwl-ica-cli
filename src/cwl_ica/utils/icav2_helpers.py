...
# Local utils
from .cwl_helper_utils import get_fragment_from_cwl_id
from .globals import ICAV2_DEFAULT_BASE_URL
from .miscell import check_shlex_arg
from .subprocess_handler import run_subprocess_proc
from .globals import ICAv2AnalysisStorageSize
...
def write_analysis_step_logs(step_logs: AnalysisStepLogs, project_id: str, log_name: str, output_path: Path, configuration: Configuration, is_cwltool_log=False):
    ...
    if log_name == "stdout":
        ...
        if hasattr(step_logs, "std_err_stream") and step_logs.std_err_stream is not None:
            ...
            raise AttributeError
    if is_stream:
        from .icav2_websocket_helpers import write_websocket_to_file, convert_html_to_text
        if is_cwltool_log:
